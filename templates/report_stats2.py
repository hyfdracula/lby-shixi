...
用法: python report_stats2.py config.yaml
by_veg=false 时跳过按植被物候 (不分类模式)。
"""
...
from src.utils import by_veg_enabled, load_config, load_lc_aligned, load_tif, veg_labels_from_config, years_in_range
...
def main() -> None:
    ...
    grid = arr0.shape[1:]
    lc = load_lc_aligned(str(data / "lc" / "lc_reclass.tif"), grid) if do_veg else None
