...
读 data_hunan + outputs_hunan, 不联网、不依赖 GEE。
用法: python make_extra_figs.py config_hunan.yaml
"""
...
def main() -> None:
    ...
    stats = by_veg_stats(cls, lc, TREND_LEVELS)
    ...
    for name in ["SOS", "EOS", "Peak"]:
        ...
        st = by_veg_stats(c, lc, PHENO_LEVELS)
        vns = list(st.keys())
